  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-neutral-800 relative" style={{ backgroundColor: theme?.bgColor || "#0a0a0a", color: theme?.textColor || "#ffffff" }}>
      ...
      <main className="max-w-md mx-auto px-6 py-8 relative z-10 space-y-8 pb-20">
        ...
        {fenixUnlocked && <div className="bg-yellow-500/10 border border-yellow-500/50 genesis-panel rounded-3xl p-5 flex items-start gap-4 animate-in zoom-in-95 duration-700 shadow-[0_0_30px_rgba(234,179,8,0.15)]"><div className="w-12 h-12 bg-yellow-500/20 rounded-full flex items-center justify-center shrink-0 border border-yellow-500/50"><Flame className="text-yellow-500" size={24} /></div><div><h2 className="text-sm font-black uppercase text-yellow-500 tracking-widest flex items-center gap-2">{text.medalTitle} <Award size={14} /></h2><p className="text-[11px] text-yellow-200/70 font-mono mt-1 leading-relaxed">{text.medalText}</p></div></div>}

        {programExpired ? <div className="bg-red-950/30 border border-red-900/50 genesis-panel rounded-3xl p-6 flex items-start gap-4"><Clock className="text-red-400 shrink-0" size={24} /><div><h2 className="text-sm font-black uppercase text-red-300 mb-1">{text.expiredTitle}</h2><p className="text-xs text-neutral-400 font-mono">{text.expiredText}</p>{programEndsAt && <p className="text-xs text-red-300 font-mono mt-2">{text.expiredOn} {new Intl.DateTimeFormat(locale === 'en' ? 'en-US' : 'es-US', { dateStyle: 'medium' }).format(new Date(programEndsAt))}</p>}</div></div> : isActive ? <div className={`genesis-surface border genesis-panel rounded-3xl p-6 relative overflow-hidden transition-all duration-700 ${fenixUnlocked ? 'border-yellow-500/50 shadow-[0_0_40px_rgba(234,179,8,0.1)]' : 'genesis-border'}`}><div className="absolute top-0 right-0 w-32 h-32 opacity-10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" style={{ backgroundColor: fenixUnlocked ? '#EAB308' : (theme?.brandColor || '#f59e0b') }} /><div className="flex justify-between items-end mb-4 relative z-10"><div><p className="text-[10px] uppercase font-black tracking-widest text-neutral-500 mb-1">{fenixUnlocked ? text.cycleCompleted : text.progress}</p><h2 className="genesis-kpi-value text-4xl font-black font-mono leading-none">{text.week} {currentWeek}<span className="text-lg text-neutral-600">/{programTotalWeeks}</span></h2></div><div className="w-12 h-12 rounded-full border-[3px] flex items-center justify-center font-black text-sm relative z-10 genesis-bg" style={{ borderColor: fenixUnlocked ? '#EAB308' : (theme?.brandColor || '#f59e0b'), color: fenixUnlocked ? '#EAB308' : (theme?.brandColor || '#f59e0b') }}>{progress}%</div></div><div className="w-full bg-neutral-900 h-2 rounded-full overflow-hidden relative z-10"><div className="h-full transition-all duration-1000 ease-out" style={{ width: `${progress}%`, backgroundColor: fenixUnlocked ? '#EAB308' : (theme?.brandColor || '#f59e0b') }} /></div></div> : <div className="bg-neutral-900 border genesis-border genesis-panel rounded-3xl p-6 flex items-start gap-4"><Clock className="text-yellow-500 shrink-0" size={24} /><div><h2 className="text-sm font-black uppercase text-white mb-1">{text.waitingTitle}</h2><p className="text-xs text-neutral-400 font-mono">{text.waitingStart} <strong className="text-white">{coachName || text.assigned}</strong> {text.waitingEnd}</p></div></div>}
