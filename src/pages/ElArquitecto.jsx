...
export default function ElArquitecto() {
  ...
  const fetchAthleteData = async () => {
    try {
      ...
      if (profile) {
        ...
        setDietStatus(profile.diet_status || 'PENDING_AUDIT');
  ...
  const chartData = {
    ...
    datasets: [
        { label: 'Grupo A (Peri-Entreno + Vinagre)', data: [0, 0.7, 1.5, 2.3, 3.0, 3.7, 4.2], borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.2)', borderWidth: 4, pointBackgroundColor: '#60a5fa', pointBorderColor: '#fff', pointHoverRadius: 8, fill: true, tension: 0.4 },
        { label: 'Grupo B (Solo Carbos Post)', data: [0, 0.4, 0.9, 1.4, 1.8, 2.2, 2.6], borderColor: '#f97316', borderWidth: 3, pointBackgroundColor: '#fb923c', pointBorderColor: '#fff', pointHoverRadius: 7, fill: false, tension: 0.4 },
        { label: 'Grupo C (Control Normal)', data: [0, 0.2, 0.4, 0.6, 0.8, 0.9, 1.1], borderColor: '#64748b', borderWidth: 2, pointBackgroundColor: '#94a3b8', pointBorderColor: '#fff', pointHoverRadius: 7, fill: false, borderDash: [5, 5], tension: 0.4 }
    ]
  };
  ...
  const exportVIPToPDF = () => {
    if(dietStatus !== 'APPROVED') return alert("Debe ser aprobado por tu coach primero.");
    ...
    doc.autoTable({ startY: 40, head: [['Día', 'Comida', 'Alimentos (Ejemplo)', 'Gr. Báscula', 'Aporte Macros', 'Justificación Clínica']], body: rows, theme: 'grid', styles: { fontSize: 8, cellPadding: 3 }, headStyles: { fillColor: [20, 20, 20], textColor: [245, 158, 11] }, columnStyles: { 0: { fontStyle: 'bold' }, 4: { fontStyle: 'bold', textColor: [59, 130, 246] } }, didParseCell: function(data) { if (data.row.index % 5 === 0 && data.section === 'body') data.cell.styles.fillColor = [240, 240, 240]; } });
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans pb-24 relative overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 relative z-10 space-y-8">
        ...
        <div className="text-center sm:text-left">
          <div className="flex items-center justify-center sm:justify-between flex-wrap gap-4 mb-2">
          ...
          <p className="text-xs text-neutral-400 font-mono">Protocolo Nutricional Clínico para: <strong className="text-white">{athlete?.goal}</strong></p>
        </div>
        ...
        <div className="bg-red-500/10 border border-red-500/30 rounded-3xl p-5 flex items-start gap-4 animate-in fade-in zoom-in duration-500 shadow-[0_0_30px_rgba(239,68,68,0.15)]">
          ...
          <div>
            ...
            <p className="text-[11px] text-red-200/80 font-mono leading-relaxed">
              El 70% de tus carbohidratos diarios deben consumirse en la ventana alrededor de tu entrenamiento (Pre y Post). <strong> El Post-Entreno exige proteína de rápida asimilación + Carbohidrato simple para cortar el cortisol inmediatamente.</strong>
            </p>
          </div>
        </div>
        ...
        {/* 👑 FUNCIÓN VIP: EJEMPLOS DE COMIDAS CON HARD-GATE DE APROBACIÓN */}
        <div className={`border rounded-3xl transition-all duration-500 shadow-xl overflow-hidden ${athlete?.b2c_plan === 'ELITE' ? 'bg-[#111] border-amber-500/30' : 'bg-black/50 border-neutral-800 opacity-60'}`}>
          <div onClick={() => athlete?.b2c_plan === 'ELITE' ? setVipExpanded(!vipExpanded) : alert("Función exclusiva para atletas VIP (Plan Élite).")} className="p-6 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors">
          ...
          {vipExpanded && athlete?.b2c_plan === 'ELITE' && (
            <div className="relative p-6 pt-0 animate-in slide-in-from-top-4 duration-300 border-t border-amber-500/20 mt-2">
              ...
              {dietStatus === 'PENDING_AUDIT' && (
                <div className="absolute inset-0 z-20 bg-[#0a0a0a]/70 backdrop-blur-[6px] flex flex-col items-center justify-center rounded-b-3xl">
                  <div className="bg-black/90 border border-amber-500/30 p-6 rounded-3xl text-center shadow-2xl max-w-sm mx-4 animate-pulse">
                    ...
                    <h3 className="text-sm font-black text-white uppercase tracking-widest mb-2">Protocolo en Auditoría</h3>
                    <p className="text-[10px] font-mono text-neutral-400 leading-relaxed">
                      El Coach <strong>{coachName}</strong> está evaluando tu biometría para hacer ajustes manuales en tus macros o prescribir suplementos.
                    </p>
                    <p className="text-[9px] text-amber-500/80 font-bold uppercase tracking-widest mt-4">
                      Se desbloqueará al ser aprobado.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2 mb-6">
                <button onClick={exportVIPToPDF} disabled={dietStatus !== 'APPROVED'} className="bg-red-500/10 text-red-500 border border-red-500/30 hover:bg-red-500 hover:text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed">
                  <FileText size={14}/> Exportar PDF
                </button>
                <button onClick={exportVIPToExcel} disabled={dietStatus !== 'APPROVED'} className="bg-green-500/10 text-green-500 border border-green-500/30 hover:bg-green-500 hover:text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed">
        ...
        <div className="bg-[#111] border border-neutral-800 rounded-3xl p-6 md:p-8 shadow-xl relative">
          ...
          {dietStatus === 'PENDING_AUDIT' && (
             <div className="absolute inset-0 z-20 bg-[#0a0a0a]/60 backdrop-blur-sm flex flex-col items-center justify-center rounded-3xl">
                ...
                <span className="text-[10px] font-black text-yellow-500 uppercase tracking-widest bg-yellow-500/10 px-3 py-1 rounded-full border border-yellow-500/20">Calculando Base Metabólica...</span>
             </div>
          )}

          <div className="flex justify-between items-center mb-6">
            <h2 className="text-sm font-black uppercase tracking-widest text-neutral-400 flex items-center gap-2"><Target size={18} style={{ color: theme?.brandColor || '#f59e0b' }}/> Requerimiento Metabólico Base</h2>
            <span className="text-xl font-black font-mono text-white bg-neutral-900 px-4 py-1 rounded-xl border border-neutral-800 shadow-inner">{macros.calories} <span className="text-[10px] text-neutral-500">KCAL</span></span>
          </div>

          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-black border border-neutral-800 rounded-2xl p-4 text-center"><span className="text-[9px] font-black uppercase tracking-widest text-blue-400 block mb-2">Proteína</span><span className="text-2xl font-black font-mono text-white">{macros.protein}g</span></div>
            <div className="bg-black border border-neutral-800 rounded-2xl p-4 text-center"><span className="text-[9px] font-black uppercase tracking-widest text-amber-500 block mb-2">Carbos</span><span className="text-2xl font-black font-mono text-white">{macros.carbs}g</span></div>
            <div className="bg-black border border-neutral-800 rounded-2xl p-4 text-center"><span className="text-[9px] font-black uppercase tracking-widest text-red-400 block mb-2">Grasas</span><span className="text-2xl font-black font-mono text-white">{macros.fats}g</span></div>
          </div>
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              {mealsData.map((m, i) => (
                <div key={i} className="bg-black border border-neutral-800 rounded-xl p-4 flex justify-between items-center hover:border-neutral-600 transition-colors">
                  <div><h3 className="text-sm font-black text-white uppercase">Comida {i + 1}</h3><p className="text-[9px] font-mono text-neutral-500 mt-1">Presupuesto asignado</p></div>
          ...
          {activeTab === 'peri' && (
            <div className="animate-in fade-in space-y-4 text-sm text-neutral-300 font-mono leading-relaxed">
              ...
              <div className="bg-blue-900/20 border border-blue-900/50 p-4 rounded-xl"><strong className="text-blue-400 block mb-1">Pre-Entrenamiento (1-2 Hrs antes)</strong>Carbohidratos complejos + Proteína magra. Cero grasas para no retrasar el vaciado gástrico.</div>
              <div className="bg-green-900/20 border border-green-900/50 p-4 rounded-xl"><strong className="text-green-400 block mb-1">Post-Entrenamiento (Inmediato)</strong>Proteína de rápida asimilación (Whey) + Carbohidrato de alto índice glucémico (Crema de arroz, banano). La insulina frenará el catabolismo.</div>
            </div>
          )}
          ...
          {activeTab === 'groceries' && (
            <div className="animate-in fade-in bg-amber-500/10 border border-amber-500/30 p-5 rounded-xl">
              ...
              <p className="text-xs text-amber-200/80 leading-relaxed font-mono">NO confundas gramos de alimento con gramos de macronutriente. El menú ya te da los gramos EXACTOS de comida a pesar en la báscula.<br/><br/>- Carnes se pesan <strong>CRUDAS</strong>.<br/>- Arroz, pasta y granos se pesan <strong>COCIDOS</strong>.<br/>- Avena y cereales se pesan <strong>CRUDOS</strong>.</p>
            </div>
          )}
        ...
        {/* 📊 LA GRÁFICA CLÍNICA RESTAURADA */}
        <div className="bg-[#111] border border-neutral-800 rounded-3xl p-6 md:p-8 shadow-xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <h2 className="text-sm font-black uppercase tracking-widest text-white flex items-center gap-2"><Activity size={18} style={{ color: theme?.brandColor || '#f59e0b' }}/> Estudio Clínico: Nutrición Peri-Entrenamiento</h2>
            ...
            <span className="text-[9px] font-black bg-neutral-900 border border-neutral-700 px-3 py-1 rounded-full text-neutral-400">Progreso 12 Semanas</span>
          </div>
          ...
          <div className="space-y-4 mb-4">
            <div className="bg-neutral-900 p-4 rounded-lg border-l-4 border-blue-500"><h4 className="font-bold text-white uppercase text-sm mb-1">Grupo A: Protocolo Élite</h4><p className="text-[11px] text-neutral-400">Dieta hipercalórica + Carbohidratos en entrenamiento + Sensibilizador de Insulina (Vinagre de Manzana previo a comida alta en carbos).</p></div>
            <div className="bg-neutral-900 p-4 rounded-lg border-l-4 border-orange-500"><h4 className="font-bold text-white uppercase text-sm mb-1">Grupo B: Protocolo Intra</h4><p className="text-[11px] text-neutral-400">Misma dieta hipercalórica, pero usando únicamente carbohidratos simples post-entreno (Sin optimizador de insulina).</p></div>
            <div className="bg-neutral-900 p-4 rounded-lg border-l-4 border-slate-500"><h4 className="font-bold text-white uppercase text-sm mb-1">Grupo C: Control Normal</h4><p className="text-[11px] text-neutral-400">Dieta estándar, comidas a deshoras, sin horario estricto alrededor del entrenamiento.</p></div>
          </div>
        </div>

        {/* 🧪 LOS HACKS METABÓLICOS */}
        <div>
          <h2 className="text-sm font-black uppercase tracking-widest text-neutral-400 mb-6 flex items-center gap-2 px-2"><Beaker size={18} style={{ color: theme?.brandColor || '#f59e0b' }}/> Hacks Metabólicos B2B</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-black border border-neutral-800 rounded-3xl p-6 relative overflow-hidden group hover:border-amber-500/50 transition-all">
              <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center border border-amber-500/30 mb-4 group-hover:scale-110 transition-transform"><Droplet className="text-amber-500" size={18}/></div>
            ...
            <div className="bg-black border border-neutral-800 rounded-3xl p-6 relative overflow-hidden group hover:border-red-500/50 transition-all">
              <div className="w-10 h-10 rounded-xl bg-red-500/10 flex items-center justify-center border border-red-500/30 mb-4 group-hover:scale-110 transition-transform"><Flame className="text-red-500" size={18}/></div>
            ...
            <div className="bg-black border border-neutral-800 rounded-3xl p-6 relative overflow-hidden group hover:border-blue-500/50 transition-all">
              <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center border border-blue-500/30 mb-4 group-hover:scale-110 transition-transform"><Zap className="text-blue-500" size={18}/></div>
