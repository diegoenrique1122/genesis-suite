-- Genesis OS 002B4.B.1B
-- Local-day correction for manual discipline check-ins.
-- The browser provides only an IANA timezone name; PostgreSQL validates it and
-- computes the canonical local date server-side. The browser never chooses log_date.
...
begin
  ...
  -- --------------------------------------------------------------------------
  -- Canonical local day.
  -- Client supplies an IANA zone name, never a date. PostgreSQL validates the
  -- zone against its timezone catalog and computes the date from server time.
  -- --------------------------------------------------------------------------
...
'Genesis 002B4.B.1B: authenticated athlete daily discipline check-in. Browser supplies only a validated IANA timezone name; PostgreSQL computes canonical local log_date and atomically writes daily_logs plus current snapshot.';
